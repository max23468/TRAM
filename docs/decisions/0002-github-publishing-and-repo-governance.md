...
TRAM viene pubblicata come repository pubblica `max23468/TRAM`, con `main` come branch predefinito.
...
`CODEOWNERS` non fa parte della baseline TRAM: in contesto single-maintainer non aggiunge controllo reale senza una decisione dedicata di branch protection/ruleset.
...
Non si usa branch protection per TRAM finché non aggiunge controllo reale al flusso single-maintainer. La governance resta basata su branch `codex/<tema>`, PR, review del maintainer, template e workflow di controllo.
...
- **Rischio: push diretto accidentale su `main`.** Mitigazione: disciplina operativa repo-first, branch `codex/<tema>` per lavori non banali, PR e controlli GitHub; branch protection/ruleset solo se verranno decisi come controllo reale.
